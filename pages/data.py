...
if data_csv is not None and data_api is not None:
    ...
    file_name = f"{csv_part}+dpe_ademe_{departement}_{'new' if neuf else 'existing'}.csv"
    with col1:
...
if data is not None:
    ...
    # Main tabs
    tab1, tab2, tab3 = st.tabs(["🧮 Datasets Print","🗺️ DPE Map", "📊 Dataset Statistics"])
    ...
    with tab3:
        ...
        col1.metric(label="Moyenne Surface Habitable", value=f"{round(surface_moyenne, 2)}m²", border=True)
        # Mean consommation énergie
        consommation_moyenne = filtered_df['cout_total_5_usages'].mean() 
        col2.metric(label="Consommation Moyenne (kWh/an)", value=f"{round(consommation_moyenne, 2)} kWh", border=True )
        # Mean cout 
        cout_moyen = filtered_df['cout_total_5_usages'].mean() 
        col3.metric(label="Coût moyen (€/an)", value=f"{round(cout_moyen, 2)}€", border=True)
        # Most frequent DPE class
        most_common_dpe = filtered_df['etiquette_dpe'].mode()[0]
        col4.metric(label="Etiquette DPE plus fréquente (replace with icon)", value=most_common_dpe, border=True)
        ...
        with col2:
            dpe_counts = filtered_df['etiquette_dpe'].value_counts().reindex(
                ["A","B","C","D","E","F","G"], fill_value=0
            )
        ...
        with col3: 
            ...
            # Count and order
            ges_counts = filtered_df['etiquette_ges'].value_counts().reindex(
                ["A", "B", "C", "D", "E", "F", "G"], fill_value=0
            )
        ...
        with col2: 
            fig = px.histogram(
                ...
                labels={"etiquette_dpe": "Classe DPE", "cout_total_5_usages": "Coût(€)"},
                text_auto=True 
            )
        ...
        with col1:
            summary_df = pd.DataFrame({
                ...
                }, index=["Moyenne", "Écart-type", "Min", "Max"]).round(2)
        ...
        with col2: 
            ...
            fig = go.Figure(go.Indicator(
                ...
                value=prop,
                number={'suffix': "%"},
                title={'text': f"Contribution de {selected_usage} <br> à la consommation énergétique",'font': {'size': 14} },
                gauge={
        ...
        with col3: 
            ...
            fig_ges = go.Figure(go.Indicator(
                mode="gauge+number",
                value=prop_ges,
                number={'suffix': "%"},
                title={'text': f"Contribution de {selected_usage} <br> aux émissions GES", 'font': {'size': 14} },
                gauge={
                    'axis': {'range': [0, 100], 'showticklabels': False},
                    'bar': {'color': "#AB63FA"}
                }
        ...
        with col1 : 
            ...
            option = st.selectbox(
                "Choix du variable Y", numeric_cols,)
            st.write("Y = ", option) 
        with col2 : 
            ...
            fig = px.scatter(
                ...
                y=option,
                labels={'conso_5_usages_ef': "Consommation Totale (kWep/m²/an)", option: option},
                title=f"Relation entre Consommation énergétique et {option} (Corrélation = {corr_value:.2f})")

            st.plotly_chart(fig, width='stretch')

else:
    st.info("⬆️ Please upload one or more CSV files or fetch ADEME API data to get started.")