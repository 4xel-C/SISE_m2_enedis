        ...
        try:
            ...
            if "Found unknown categories" in msg:
                ...
                if "in column" in msg:
                    ...
                    bad_col = (
                        features[int(col_part)] if col_part.isdigit() else col_part
                    )
                else:
                    bad_col = features[0]
...
st.write(
    "Select a dataset in the **sidebar**, then click the button to retrain both models on the new data."
)
...
if st.button("🚀 Retrain models"):
    with st.spinner("Processing..."):
        ...
        with open(
            os.path.join(MODELS_DIR, "features_target_columns_regression.pkl"), "rb"
        ) as f:
            reg_info = pickle.load(f)
        with open(
            os.path.join(MODELS_DIR, "features_target_columns_classification.pkl"), "rb"
        ) as f:
            clf_info = pickle.load(f)

        reg_features = (
            reg_info["quantitative_features"] + reg_info["qualitative_features"]
        )
        reg_target = reg_info["target"]
        clf_features = (
            clf_info["quantitative_features"] + clf_info["qualitative_features"]
        )
        ...
        with st.spinner("Training regression model..."):
            ...
            reg_pipeline_orig = load_model(
                os.path.join(MODELS_DIR, "pipeline_best_regression.pkl")
            )
            X_reg, y_reg = df[reg_features], df[reg_target]

            # Evaluate original model (with filtering to avoid errors)
            df_reg_filtered, unknowns_reg = filter_dataset_for_original(
                df, reg_pipeline_orig, reg_features
            )
            if unknowns_reg:
                st.warning(
                    f"Regression: {len(df) - len(df_reg_filtered)} rows ignored for original model "
                    f"(unknown categories: {unknowns_reg})"
                )

            if len(df_reg_filtered) > 0:
                X_reg_f, y_reg_f = (
                    df_reg_filtered[reg_features],
                    df_reg_filtered[reg_target],
                )
                y_pred_orig = reg_pipeline_orig.predict(X_reg_f)
                metrics_reg_orig = {
                    "R2": r2_score(y_reg_f, y_pred_orig),
            ...
            reg_pipeline_retrained = load_model(
                os.path.join(MODELS_DIR, "pipeline_best_regression.pkl")
            )
            make_pipeline_tolerant_inplace(reg_pipeline_retrained)
            reg_pipeline_retrained.fit(X_reg, y_reg)

            retrain_reg_path = os.path.join(
                MODELS_DIR, "pipeline_best_regression_retrained.pkl"
            )
            ...
            st.success(
                f"✅ Regression retrained in {duration/60:.2f} min (tolerant to unseen categories)"
            )
        ...
        with st.spinner("Training classification model..."):
            ...
            df_clf_filtered, unknowns_clf = filter_dataset_for_original(
                df, clf_pipeline_orig, clf_features
            )
            if unknowns_clf:
                st.warning(
                    f"Classification: {len(df) - len(df_clf_filtered)} rows ignored for original model "
                    f"(unknown categories: {unknowns_clf})"
                )

            if len(df_clf_filtered) > 0:
                X_clf_f, y_clf_f = (
                    df_clf_filtered[clf_features],
                    df_clf_filtered[clf_target],
                )
            ...
            clf_pipeline_retrained = load_model(
                os.path.join(MODELS_DIR, "pipeline_xgboost_classification.pkl")
            )
            make_pipeline_tolerant_inplace(clf_pipeline_retrained)
            clf_pipeline_retrained.fit(X_clf, y_clf_encoded)

            retrain_clf_path = os.path.join(
                MODELS_DIR, "pipeline_xgboost_classification_retrained.pkl"
            )
            ...
            duration = time.time() - start
            st.success(
                f"✅ Classification retrained in {duration/60:.2f} min (tolerant to unseen categories)"
            )
    ...
    st.success(
        "🎉 All models retrained successfully and are now tolerant to unseen categories."
    )
